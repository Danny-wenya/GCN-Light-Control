...
def gcn_input(state):
    for st in state:
        st["GCNflow"]=np.array(st["GCNflow"])
        tp=np.repeat(np.array([st["TSprevphases"]]),axis=0,repeats=st["GCNflow"].shape[0])
        tn=np.repeat(np.array([st["turn"]]).T,axis=1,repeats=st["GCNflow"].shape[1])
        st["GCNinput"]=np.stack([st["GCNflow"],tp,tn],axis=-1)
    return state