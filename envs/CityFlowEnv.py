    ...
    def __init__(self, log_path, work_folder, config, logfile = '', seed = 0, 
        ...
        for num, inter in enumerate(self.list_intersection):
            ...
            for rnum, rname in enumerate(inter.roadnames):
                self.roadname2id2node[rname] = [num, rnum,node]
                node+=1
        for ID in virtual_inters:
            ...
            self.roadname2id2node[roadnames[0]] = [-1 - num, 0,-node]
            node+=1

        for inter in self.list_intersection:
            inter._set_road_links_in(self.roadname2id2node)
            for rl in inter.roadnet_info['roadlinks']:
                if rl['startRoad'] in self.roadname2id2node:
                    num, rnum,_ = self.roadname2id2node[rl['startRoad']]
                    if num < 0:
                        vid = -1 - num
