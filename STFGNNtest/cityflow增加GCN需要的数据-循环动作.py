...
# 循环动作，每个相位的执行时长相等，都为20s。先随机初始化一个动作，每次迭代在原动作基础上+1.
def cycling_action(actions):
    next_ac=[[int((i[0]+1)%8)] for i in actions]
    return next_ac
...
if __name__=="__main__":
    ...
    t0=time.time()
    # with open('X_Y循环动作.json','a') as f:
    for i in range(epochs) :
        actions=cycling_action(actions)
    ...
    logging.info(f"In {pythonfilename}")
    logging.info(f"删除了原UniLight中STFGNN不需要的数据收集操作，增加STFGNN需要的数据收集,采用随机动作。epochs:{epochs},time_span:{t1-t0:.2f}s,evrage time span every epoch:{(t1-t0)/epochs:.2f}s")
